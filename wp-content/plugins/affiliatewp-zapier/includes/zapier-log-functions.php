 ...
 * @param $row_id                    Row ID.
 * @return mixed   log object|false  Returns a log object if successful, otherwise false.
 * @since 1.0
 */
function affwp_zapier_post_update_visit( $data, $row_id ) {

	$log_data = array();

	if ( empty( $data ) ) {
		affiliate_wp()->utils->log( 'affwp_zapier_post_update_visit: No visit data was provided.' );
		return false;
	}

	$visit_id = ! empty( $row_id ) ? absint( $row_id ) : absint( $data['visit_id'] );

	if ( ! $visit_id ) {
		affiliate_wp()->utils->log( 'affwp_zapier_post_update_visit: Unable to retrieve visit ID.' );
		return false;
	}
	...
	if ( ! $visit ) {
		affiliate_wp()->utils->log( 'affwp_zapier_post_update_visit: Could not retrieve visit object for ID: ' . $row_id );
		return false;
	}
	...
	$log_data['object_id'] = $visit_id;
	$log_data['meta']      = json_encode( $meta );
	$log_data['object']    = 'visit';
	$log_data['action']    = 'update';

	$log = affwp_zapier_add_log( $log_data, 'visit' );

	/**
	 * Fires immediately after a visit-update log is created.
	 *
	 * @param $log_data
	 * @since 1.0
	 */
	do_action( 'affwp_zapier_after_add_visit_log', $log_data );
